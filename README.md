...
awescholar render agentx --archive data.json -o intake.sh --emit commands  # Shell script of pnpm agent:add lines instead of candidate JSON
awescholar render agentx --archive data.json -o intake.sh --emit commands --exclude-snapshot agents-snapshot.json --llm-category  # LLM-picked category slugs that exist in the snapshot
